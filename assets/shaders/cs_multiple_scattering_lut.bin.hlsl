...
{
float phi = Xi.y * 2.0 * 3.141592653589793;
...
float3 sampleGGX(float2 Xi, float a)
{
float phi = 2.0 * 3.141592653589793 * Xi.y;
...
float albedo_specular(float3 V, float NoV, PBRMaterial mat)
{
const float3 N = float3(0.0, 1.0, 0.0);
float E = 0.0;
for(uint i = 0; i < 1024; i++)
{
float2 Xi = hammersley_2d(i, 1024);
float3 H = sampleGGX(Xi, mat.a);
...
float albedo_diffuse(float3 V, float NoV, PBRMaterial mat)
{
float E = 0.0;
for(uint i = 0; i < 1024; i++)
{
float2 Xi = hammersley_2d(i, 1024);
float3 L = sampleHemisphereCosine(Xi);
...
{
int2 coords = int2(gl_GlobalInvocationID.xy);
float2 values = (float2(coords) + 0.5) / 32;
float NoV = values.x;
float roughness = sqrt(values.y);
float3 V;
V.x = sqrt(1.0 - NoV * NoV);
V.y = NoV;
V.z = 0.0;
PBRMaterial mat;
mat.normal = float3(0.0, 1.0, 0.0);
mat.occlusion = 0.0;
