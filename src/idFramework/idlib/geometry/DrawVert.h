...
ID_INLINE void idDrawVert::Lerp( const idDrawVert &a, const idDrawVert &b, const float f ) {
	...
	SetTexCoord( idMath::Lerp( a.GetTexCoord(), b.GetTexCoord(), f ) );
}
...
ID_INLINE void idDrawVert::LerpAll( const idDrawVert &a, const idDrawVert &b, const float f ) {
	xyz = idMath::Lerp( a.xyz, b.xyz, f );
	SetTexCoord( idMath::Lerp( a.GetTexCoord(), b.GetTexCoord(), f ) );

	idVec3 normal = idMath::Lerp( a.GetNormal(), b.GetNormal(), f );
	idVec3 tangent = idMath::Lerp( a.GetTangent(), b.GetTangent(), f );
	idVec3 bitangent = idMath::Lerp( a.GetBiTangent(), b.GetBiTangent(), f );
	normal.Normalize();
	tangent.Normalize();
	bitangent.Normalize();
	SetNormal( normal );
