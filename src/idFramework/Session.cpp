...
#include "idFramework/Session_local.h"
...
void idSessionLocal::ShowLoadingGui() {
	...
	unsigned int stop = Sys_Milliseconds() + 1000;
	int force = 10;
...
#ifdef _WIN32 
	// If Editor is minimized (no editor active), and game window is not active, dont run game, we still take over 
	else if (com_editors)
		if ( !Sys_IsWindowVisible() ) {
			return;
		}
