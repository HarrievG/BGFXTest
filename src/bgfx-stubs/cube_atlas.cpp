...
{
	common->DPrintfIf(_width < 2, "_width must be > 2");
	common->DPrintfIf(_height < 2, "_height must be > 2");
	m_width = _width;
	m_height = _height;
	m_usedSpace = 0;
...
{
	common->DPrintfIf(!(_textureSize >= 64 && _textureSize <= 4096), "Invalid _textureSize %d.", _textureSize);
	common->DPrintfIf(!(_maxRegionsCount >= 64 && _maxRegionsCount <= 32000), "Invalid _maxRegionsCount %d.", _maxRegionsCount);
	...
	bgfx::setName(m_textureHandle,"cube_atlas");
}
...
{
	common->DPrintfIf(!(_regionCount <= 64 && _maxRegionsCount <= 4096), "_regionCount %d, _maxRegionsCount %d", _regionCount, _maxRegionsCount);
