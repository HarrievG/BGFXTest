...
public:
	...
	*/
	...
	/// position and color.
	void appendText(const char* _string, const char* _end = NULL);

	/// Append a wide char unicode string to the buffer using current pen
	/// position and color.
	void appendText(const wchar_t* _string, const wchar_t* _end = NULL);
...
private:
	void appendGlyph(CodePoint _codePoint, bool shadow);
...
TextBuffer::TextBuffer(idFont* _font)
...
void TextBuffer::appendText(const char* _string, const char* _end)
{
	if (m_vertexCount == 0)
	{
		m_originX = m_penX;
...
void TextBuffer::appendText(const wchar_t* _string, const wchar_t* _end)
{
	if (m_vertexCount == 0)
	{
		m_originX = m_penX;
...
void TextBuffer::appendAtlasFace(uint16_t _faceIndex)
{
	...
	float x0 = m_penX;
	float y0 = m_penY;
	float x1 = x0 + (float)m_font->GetAtlas()->getTextureSize();
	float y1 = y0 + (float)m_font->GetAtlas()->getTextureSize();

	m_font->GetAtlas()->packFaceLayerUV(_faceIndex
...
void TextBuffer::appendGlyph(CodePoint _codePoint, bool shadow)
{
	if (_codePoint == L'\t')
	{
		for (uint32_t ii = 0; ii < 4; ++ii)
		{
			appendGlyph( L' ', shadow);
		}
		return;
	}
	...
	const FontInfo& font =idFont::GetFontInfo(fontHash);
	...
	float kerning = 1.0f;//m_fontManager->getKerning(_handle, m_previousCodePoint, _codePoint);
	m_penX += kerning;

	const GlyphInfo& blackGlyph = idFont::blackGlyph;
	const Atlas* atlas = m_font->GetAtlas();//m_fontManager->getAtlas();
	...
	{
		float x0 = (m_penX - kerning);
		float y0 = (m_penY + m_lineAscender + font.underlineThickness);
		float x1 = ( (float)x0 + (glyph->advance_x) );
	...
	&&  m_overlineColor & 0xFF000000)
	{
		...
		m_font->GetAtlas()->packUV(blackGlyph.regionIndex
			, (uint8_t*)m_vertexBuffer
			, sizeof(TextVertex) * m_vertexCount + offsetof(TextVertex, u)
			, sizeof(TextVertex)
			);
	...
	else if (!shadow)
	{
		...
		float glyphWidth = glyph->width * glyphScale;
		float glyphHeight = glyph->height * glyphScale;
		float x0 = m_penX + ( glyph->offset_x ) - 1.0;
		float y0 = (m_penY + (glyph->offset_y) ) ;
		float x1 = ( x0 + glyph->width );
		float y1 = ( y0 + glyph->height );
...
TextBufferManager::TextBufferManager(idFont * _font) : m_font(_font )
{
...
void TextBufferManager::submitTextBuffer(TextBufferHandle _handle, bgfx::ViewId _id, int32_t _depth)
{
	...
	bgfx::setTexture(0, s_texColor, m_font->GetAtlas()->getTextureHandle() );

	auto * atlas =  m_font->GetAtlas();
	...
	case FONT_TYPE_DISTANCE:
	{
		...
		float params[4] = { 0.0f, (float) atlas->getTextureSize() / 512.0f, 0.0f, 0.0f };
	...
	case FONT_TYPE_DISTANCE_OUTLINE:
	{
		...
		float params[4] = { 0.0f, (float)atlas->getTextureSize() / 512.0f, 0.0f, bc.textBuffer->getOutlineWidth() };
		bgfx::setUniform(u_params, &params);
		break;
	}
	...
	case FONT_TYPE_DISTANCE_DROP_SHADOW:
	{
		...
		float params[4] = { 0.0f, (float)atlas->getTextureSize() / 512.0f, bc.textBuffer->getDropShadowSoftener(), 0.0 };
		bgfx::setUniform(u_params, &params);
		break;
	}
	...
	case FONT_TYPE_DISTANCE_OUTLINE_DROP_SHADOW_IMAGE:
	{
		...
		float params[4] = { 0.0f, (float)atlas->getTextureSize() / 512.0f, bc.textBuffer->getDropShadowSoftener(), bc.textBuffer->getOutlineWidth() };
		bgfx::setUniform(u_params, &params);
		break;
	}
...
void TextBufferManager::appendText(TextBufferHandle _handle, const char* _string, const char* _end)
{
	BX_ASSERT(isValid(_handle), "Invalid handle used");
	BufferCache& bc = m_textBuffers[_handle.idx];
	bc.textBuffer->appendText(_string, _end);
}

void TextBufferManager::appendText(TextBufferHandle _handle, const wchar_t* _string, const wchar_t* _end)
{
	BX_ASSERT(isValid(_handle), "Invalid handle used");
	BufferCache& bc = m_textBuffers[_handle.idx];
	bc.textBuffer->appendText(_string, _end);
