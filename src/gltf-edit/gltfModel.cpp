...
#include "idFramework/idlib/Dict.h"
...
//                boundingBox.max = glm::vec3{ transform * glm::vec4{ boundingBox.max, 1.0f } };
...
//                meshGroup->materials.push_back(materialModePair.first);
//                meshGroup->transforms.push_back(transform);
//                meshGroup->boundingBoxes.push_back(boundingBox);
//            }
//        }
//    }
//
...
bgfxModel loadGltfModel(const idStr &fileName)
{
    ...
    bool res = loader.LoadASCIIFromFile(&gltf_model, &err, &warn, fileName.c_str());
    ...
    if (!res)
        common->FatalError("Failed to load GLTF Model");
    ...
    output_model.textures.AssureSize(gltf_model.textures.size() + DUMMY_TEXTURE_COUNT);
    ...
    {
        const std::string& uri =  gltf_model.images[texture.source].uri;
