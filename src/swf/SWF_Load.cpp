...
bool idSWF::LoadSWF( const char * fullpath ) {
	...
	if ( rawfile == NULL ) {
		common->Printf( "SWF File not found %s\n", fullpath );
		return false;
	}
...
bool idSWF::LoadBinary( const char * bfilename, ID_TIME_T sourceTime ) {
	...
	for ( int i = 0; i < dictionary.Num(); i++ ) {
		...
		switch ( dictionary[i].type ) {
			...
			case SWF_DICT_MORPH:
			case SWF_DICT_SHAPE: {
				dictionary[i].shape = new  idSWFShape;
			...
			case SWF_DICT_SPRITE: {
				dictionary[i].sprite = new  idSWFSprite( this );
				dictionary[i].sprite->Read( f );
				break;
			}
			case SWF_DICT_FONT: {
				dictionary[i].font = new  idSWFFont;
			...
			case SWF_DICT_TEXT: {
				dictionary[i].text = new  idSWFText;
			...
			case SWF_DICT_EDITTEXT: {
				dictionary[i].edittext = new  idSWFEditText;
