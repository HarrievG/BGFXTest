...
namespace avmplus
{
    ...
    {
        assert(atomIsIntptr(a));
        assert(atomIsValidIntptrValue(intptr_t(a) >> 3));
        return intptr_t(a) >> 3;
    }
    ...
    {
        assert(avmplus::testIsObject(a)); // proper type and not null or undefined
        return (avmplus::ScriptObject*) (uintptr_t(a) - kObjectType);
    }

    REALLY_INLINE bool atomGetBoolean(Atom a)
    {
        assert(a == trueAtom || a == falseAtom);
        return (a >> 3) != 0;
    }
    ...
    {
        assert(atomKind(a) == kSpecialBibopType);
        uint8_t const k = *(uint8_t*)(uintptr_t(a) & kBibopTypePtrMask);
        assert(k == kBibopUndefined || k == kBibopFloatType  || k == kBibopFloat4Type);
        return k;
    }
#endif
