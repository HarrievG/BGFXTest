		...
		switch ( code ) {
			...
			case Action_NextFrame:
				if ( verify( currentTarget != NULL ) ) {
					...
					common->Printf( "SWF: no target movie clip for nextFrame\n" );
				}
				break;
			case Action_PrevFrame:
				if ( verify( currentTarget != NULL ) ) {
					...
					common->Printf( "SWF: no target movie clip for prevFrame\n" );
				}
				break;
			case Action_Play:
				if ( verify( currentTarget != NULL ) ) {
					...
					common->Printf( "SWF: no target movie clip for play\n" );
				}
				break;
			case Action_Stop:
				if ( verify( currentTarget != NULL ) ) {
					...
					common->Printf( "SWF: no target movie clip for stop\n" );
				}
				break;
			...
			case Action_GotoFrame: {
				...
				if ( verify( currentTarget != NULL ) ) {
					...
					common->Printf( "SWF: no target movie clip for runTo %d\n", frameNum );
				}
				break;
			}
			case Action_SetTarget: {
				...
				if ( verify( thisSprite != NULL ) ) {
					...
					common->Printf( "SWF: no target movie clip for setTarget %s\n", targetName );
				}
				break;
			}
			case Action_GoToLabel: {
				...
				if ( verify( currentTarget != NULL ) ) {
					...
					common->Printf( "SWF: no target movie clip for runTo %s\n", targetName );
				}
				break;
			}
			case Action_Push: {
			...
			case Action_GetVariable: {
				...
				if ( stack.A().IsUndefined() && swf_debug.GetInteger() > 1 ) {
					common->Printf( "SWF: unknown variable %s\n", variableName.c_str() );
				}
				break;
			}
			...
			case Action_GotoFrame2: {
				...
				if ( verify( thisSprite != NULL ) ) {
					...
					if ( ( flags & 1 ) != 0 ){
						common->Printf( "SWF: no target movie clip for gotoAndPlay\n" );
					} else {
						common->Printf( "SWF: no target movie clip for gotoAndStop\n" );
					}
				}
				stack.Pop( 1 );
				break;
			...
			case Action_GetProperty: {
				if ( verify( thisSprite != NULL ) ) {
					...
					common->Printf( "SWF: no target movie clip for getProperty\n" );
				}
				stack.Pop( 1 );
				break;
			}
			case Action_SetProperty: {
				if ( verify( thisSprite != NULL ) ) {
					...
					common->Printf( "SWF: no target movie clip for setProperty\n" );
				}
				stack.Pop( 3 );
				break;
			}
			...
			case Action_GetMember: {
				if ( ( stack.B().IsUndefined() || stack.B().IsNULL() ) && swf_debug.GetInteger() > 1 ) {
					common->Printf( "SWF: tried to get member %s on an invalid object in sprite '%s'\n", stack.A().ToString().c_str(), thisSprite != NULL ? thisSprite->GetName() : "" );
				}
				if ( stack.B().IsObject() ) {
					...
					if ( stack.B().IsUndefined() && swf_debug.GetInteger() > 1 ) {
						common->Printf( "SWF: unknown member %s\n", stack.A().ToString().c_str() );
					}
			...
			case Action_With: {
				...
				if ( stack.A().IsObject() ) {
					...
					if ( swf_debug.GetInteger() > 0 ) {
						common->Printf( "SWF: with() invalid object specified\n" );
					}
					stack.Pop( 1 );
				}
				break;
			...
			case Action_Delete: {
				if ( swf_debug.GetInteger() > 0 ) {
					common->Printf( "SWF: Delete ignored\n" );
				}
			...
			case Action_Delete2: {
				if ( swf_debug.GetInteger() > 0 ) {
					common->Printf( "SWF: Delete2 ignored\n" );
				}
				// We no longer support deleting variables because the performance cost of updating the hash tables is not worth it
				stack.Pop( 1 );
				break;
...
void idSWF::Invoke( const char * functionName, const idSWFParmList & parms ) {
	...
	if ( swf_debugInvoke.GetBool() ) {
		common->Printf( "SWF: Invoke %s with %d parms (%s)\n", functionName, parms.Num(), GetName() );
	}
