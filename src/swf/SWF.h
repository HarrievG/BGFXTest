...
#include "../idFramework/idlib/containers/Sort.h"
...
public:
	...
	int PlayShaderDirectly( const char *sound, int channel ) {
		return 0;
	}
...
public:
	...
	int PlaySound( const char * sound, int channel = 1/*SCHANNEL_ANY*/, bool blocking = false );
	void StopSound( int channel = 1/*SCHANNEL_ANY*/ );
...
#define SWF_NATIVE_FUNCTION_SWF_DECLARE( x ) \
	...
	idList< idSWFDictionaryEntry >	dictionary;
	...
	idList< keyButtonImages_t > tooltipButtonImage;
	...
	idList< tooltipIcon_t > tooltipIconList;
...
private:
	...
	idList<imageToPack_t >	packImages;	// only used during creation
